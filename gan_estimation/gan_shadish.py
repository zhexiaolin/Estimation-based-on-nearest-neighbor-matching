...
def do_all(df, type, batch_size=128, architecture = [128, 128, 128], lr=1e-4,
    ...
    critic_arch.reverse()
    # X | t
    continuous_vars1 = ["vocabpre", "mathpre", "numbmath", "age", "momdegr", "daddegr", "hsgpaar"]
    continuous_lower_bounds1 = {"vocabpre": 0, "mathpre": 0, "numbmath": 0, "age": 0, "momdegr": 0, "daddegr": 0, "hsgpaar": 0}
    categorical_vars1 = ["cauc", "male"]
    context_vars1 = ["vm"]

    # Y | X, t
    continuous_vars2 = ["mathall"]
    continuous_lower_bounds2 = {"mathall": 0}
    context_vars2 = ["vm", "vocabpre", "mathpre", "numbmath", "age", "momdegr", "daddegr", "hsgpaar", "cauc", "male"]

    df_balanced = df.sample(2*len(df), weights=(1-df.vm.mean())*df.vm+df.vm.mean()*(1-df.vm),
    ...
    df_fake_x["vm"] = 1 - df_fake_x["vm"]
    df_fake["mathall_cf"] = data_wrappers[1].apply_generator(G[1], df_fake_x)["mathall"]
    ...
    scatterplot = dict(x=[],
                         y=[],
                         samples = 400)
    histogram = dict(variables=["mathall", "vm", "vocabpre", "mathpre", "numbmath", "age", "momdegr", "daddegr", "hsgpaar", "cauc", "male"],
                       nrow=3, ncol=3)
